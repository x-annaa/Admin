...
async function loadProducts() {
  ...
  data.forEach(p => {
    ...
    tr.innerHTML = `
      ...
      <td>${p.profit}</td>
      <td>
        <button onclick="openProductModal(${p.id}, '${p.name}', ${p.price}, '${p.description}', ${p.profit})">✏ 编辑</button>
        <button onclick="openProductMatchModal(${p.id}, ${p.enabled}, ${p.manual_only})">🎯 匹配</button>
      </td>
    `;
